...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_sql = "UPDATE laporan SET solusi = '$new_status' WHERE id = $id_pengaduan";
    $update_result = mysqli_query($connection, $update_sql);

    // Periksa keberhasilan pembaruan
    if ($update_result) {
        echo "Solusi berhasil diperbarui.";
    } else {
        echo "Error: " . mysqli_error($connection);
    }
}
        ...
        <?php while ($data = mysqli_fetch_assoc($result)) { ?>
            <tr>
                ...
                <td>
                    ...
                    <!-- Formulir untuk mengubah status -->
<form action="solusi_piber.php" method="post">
    <input type="hidden" name="id_pengaduan" value="<?= $data['id']; ?>">
    <input type="text" name="new_status" autocomplete="off" placeholder="Masukkan solusi baru">
    <button type="submit">Ubah Solusi</button>
</form>
